...
client.on("connection", socket => {
  ...
  socket.on("insert_number", data => {
    ...
    if (data.user >= 25)
      connection.query(
        `INSERT INTO numbers(person_id,number) VALUES(${data.user}, ${Number(
          data.number
        )})`,
  ...
  socket.on("find", data => {
    ...
    connection.query(
      `SELECT number FROM numbers WHERE number=${data.target}`,
      (err, num) => {
        // console.log()
        if (Array.from(num).length === 0) {
          ...
          connection.query(
            `SELECT numbers.person_id FROM numbers WHERE number=${data.target}`,
            (err, pid) => {
              // if(err) console.log('oh no!!')
              // console.log(pid[0].person_id)
              connection.query(
                `SELECT users.name FROM users WHERE id=${pid[0].person_id}`,
                (err, res) => {
                  // if(err) console.log('oh no!!')
    ...
    connection.query(
      ...
      (err, num) => {
        ...
        if (Array.from(num).length === 0) {
          ...
          connection.query(
            `SELECT users.id FROM users WHERE name="${data.name}"`,
            (err, pid) => {
              // if(err) console.log('oh no!!')
              // console.log(pid[0].person_id)
              connection.query(
                `SELECT numbers.number FROM numbers WHERE person_id=${pid[0].id}`,
                (err, res) => {
                  // if(err) console.log('oh no!!')
                  // socket.emit('result', res)
                  // console.log(res)
